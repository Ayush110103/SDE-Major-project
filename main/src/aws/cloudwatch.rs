...
use chrono::{DateTime, Duration as Delta, Local, NaiveDateTime, TimeZone, Utc};
...
// Updated calculate_start_time and print_date for deprecated functions
...
fn calculate_start_time(from: DateTime<Local>, delta: Duration) -> Option<i64> {
    ...
    let utc_time = Utc.from_utc_datetime(&start_time.naive_utc());
...
fn print_date(time: Option<i64>) -> String {
    match time {
        Some(x) => Local
            .from_utc_datetime(&NaiveDateTime::from_timestamp_opt(x / 1000, 0).unwrap())
            .format("%Y-%m-%d %H:%M:%S")
            .to_string(),
        None => "".to_owned(),
    }
